...
const turnEnd = async () => {
  ...
  const characterList = getCharacterList().map(x => {
      ...
      const requiredAttributes = [
          ...
          'stackDamageUp', 'stackProtection', 'stackPowerUp','stackFEOAwaken','stackbiribiri',
          'stackPoison', 'isPlayer', 'checkseikou', 'stackBleeding','stackDarkFire'
      ];
  ...
  const promises = characterList.map(x => {
      return [
          ...
          x.entity.modifyTokenAttribute('attributes.constitution', x.constitution),
          x.entity.modifyTokenAttribute('attributes.stackSinsyoku', x.stackSinsyoku),
          ...
          x.entity.modifyTokenAttribute('attributes.stackParalysis', x.stackParalysis),
          x.entity.modifyTokenAttribute('attributes.stackFear', x.stackFear),
          x.entity.modifyTokenAttribute('attributes.stackDamageDown', x.stackDamageDown),
          x.entity.modifyTokenAttribute('attributes.stackVulnerable', x.stackVulnerable),
          x.entity.modifyTokenAttribute('attributes.stackPowerDown', x.stackPowerDown),
          x.entity.modifyTokenAttribute('attributes.stackDamageUp', x.stackDamageUp),
          x.entity.modifyTokenAttribute('attributes.stackProtection', x.stackProtection),
          x.entity.modifyTokenAttribute('attributes.stackPowerUp', x.stackPowerUp),
          x.entity.modifyTokenAttribute('attributes.stackPoison', x.stackPoison),
          x.entity.modifyTokenAttribute('attributes.stackBleeding', x.stackBleeding),
          x.entity.modifyTokenAttribute('attributes.stackDarkFire', x.stackDarkFire),
          x.entity.modifyTokenAttribute('attributes.stackFEOAwaken', x.stackFEOAwaken),
      ];
  }).flat();
...
const turnStart = async () => {
  ...
  // HP更新
  await Promise.all(characterList.map(x => {
...
const applyBiribiriWhenTurnStart = async (characterList) => {
  ...
  .map(x => {
      ...
      resultList.push(x.entity.modifyTokenAttribute('attributes.stackbiribiri', beforeStack + AddStack));
      resultList.push(x.entity.modifyTokenAttribute('attributes.stackbiribirinext', 0));
      return x;
  });
};
...
const applyAwakenWhenTurnStart = async (characterList) => {
  let resultList = [];
  characterList
  .filter(x => x.entity.system.attributes.stackFEOAwakenNext && x.entity.system.attributes.stackFEOAwakenNext.value > 0)
  .map(x => {
      console.log(x.entity.name + "に対して充電追加処理")
      const beforeStack = x.entity.system.attributes.stackFEOAwaken ? x.entity.system.attributes.stackFEOAwaken.value : 0;
      const AddStack = x.entity.system.attributes.stackFEOAwakenNext.value;
      resultList.push(x.entity.modifyTokenAttribute('attributes.stackFEOAwaken', beforeStack + AddStack));
      resultList.push(x.entity.modifyTokenAttribute('attributes.stackFEOAwakenNext', 0));
      return x;
  });
};
...
const resetDarkFireWhenTurnStart = async (characterList) => {
  ...
  characterList.forEach(x => {
      ...
      resetPromises.push(x.entity.modifyTokenAttribute('attributes.stackDarkFire', 0));
  });
